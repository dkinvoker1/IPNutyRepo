    ...
    public static class OrdersCollection
    {
        public static List<Order> GetAllOrders()
        {
            using (ApplicationDbContext dbcontext = new ApplicationDbContext())
            {
                ...
                List<Singer> singersWhoOrder = new List<Singer>();

                List<SheetMusic> sheetMusicList = db1.SheetsOfMusic.ToList();
                List<SheetMusic> sheetMusicOrdered = new List<SheetMusic>(); //sheetMusicList.Take(ordersList.Count).ToList();

                for (int i = 0; i <= ordersList.Count - 1; i ++)
                {
                    singersWhoOrder.Add(singersList.Where(e => e.SingerId == ordersList[i].SingerId.SingerId).FirstOrDefault());
                    sheetMusicOrdered.Add(sheetMusicList.Where(e => e.SheetMusicId == ordersList[i].SheetMusicId.SheetMusicId).FirstOrDefault());
                }
